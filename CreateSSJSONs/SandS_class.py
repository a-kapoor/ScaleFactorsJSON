...
import numpy as np
class SandS:
    def __init__(self,runbins,fulldict,smearingdict):
        
        self.runbins=runbins
        self.df=fulldict
        self.dfsmearing=smearingdict
        self.etabins=[float('-inf'),-2.0,-1.566,-1.442, -1.2,-1, 0.0, 1,1.2, 1.442, 1.566,2.0,float('inf')]
        ...
        self.etabinssmearing=[float('-inf'),-2.0,-1.566,-1.442,-1, 0.0, 1, 1.442, 1.566,2.0,float('inf')]
        self.etabinssmearingstr=['EE3','EE2','EE1','EB2','EB1','EB1','EB2','EE1','EE2','EE3']
        self.etassmearing={'EE3':[2.0,2.5],'EE2':[1.566,2.0],
                   'EE1':[1.566,2.0],#'EE1':[1.442,1.566],EE1 needs to be defined properly. currently substituted
                   'EB2':[1.0,1.442],
                   'EB1':[0.0,1.0]}
    ...
    def getSmearing(self,etas,r9bin,valtype,year):
        try:
            if etas[0]==1.442:
                return 0
            filtered_rows = self.dfsmearing[
                (self.dfsmearing['abseta_min'] == etas[0]) &
                (self.dfsmearing['abseta_max'] == etas[1]) &
                (self.dfsmearing['R9_min'] == r9bin[0]) &
                (self.dfsmearing['R9_max'] == r9bin[1])]
            #print(filtered_rows.iloc[0][valtype])
            return filtered_rows.iloc[0][valtype]
        except:
            #print("addingzero")
            #print(etas,r9bin,valtype,year)
            return 0
    ...
    def getrunSmearingnew(self,valtype,year):
        return self.getmultibin("eta",self.etabinssmearing,[
            self.getmultibin("r9",[0.0,0.96,10.0],[
                self.getSmearing(self.etassmearing[strbin],r9bin,valtype,year)
                for r9bin in [[0.0,0.96],[0.96,10.0]]])
            for strbin in self.etabinssmearingstr])
