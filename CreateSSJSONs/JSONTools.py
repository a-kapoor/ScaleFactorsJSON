...
def CSEVSFs(files,name,i,IsSF="sf",run=2):
    file=TFile(files[name])
    if run==3:
        hname="hsp_CSEV_"+name
        if name=='MVA':
            hname="hsp_CSEV_MVA90"
    else:
        hname=name+"ID/SF_CSEV_"+name+"ID"
    hist=file.Get(hname)
    SF=0
    if IsSF=="sf":
...
def CSEVSFyearwise(files=[],names=[],valtypes=["sf","sfup","sfdown"],run=2):
                                ...
                                schema.CategoryItem.parse_obj({
                                        ...
                                        "value": schema.Category.parse_obj({
                                            ...
                                            "content": [schema.CategoryItem.parse_obj({"key": binlist[i-1], 
                                                                                       "value": CSEVSFs(files,name,i,val,run)})
                                                        for i in range(1,7)
                                                       ],
                                        })
                                })
    ...
    return output
def HasPixSFs(files,name,i,IsSF="sf",run=2):
    file=TFile(files[name])
    if run==3:
        hname="hsp_HasPix_"+name
        if name=='MVA':
            hname="hsp_HasPix_MVA90"
    else:
        hname=name+"ID/SF_CSEV_"+name+"ID"    
...
def HasPixSFyearwise(files=[],names=[],valtypes=["sf","sfup","sfdown"],run=2):
    binlist=['EBInc','EBHighR9','EBLowR9','EEInc','EEHighR9','EELowR9']
    output = schema.Category.parse_obj({
                "nodetype": "category",
                "input": "ValType",
                            ...
                            "content":[
                                schema.CategoryItem.parse_obj({
                                        ...
                                        "value": schema.Category.parse_obj({
                                            ...
                                            "input": "HasPixBin",
                                            "content": [schema.CategoryItem.parse_obj({"key": binlist[i-1], 
                                                                                       "value": HasPixSFs(files,name,i,val,run)})
                                                        for i in range(1,7)
                                                       ],
                                        })
                                })
...
def getSSdictRun2(year,url_dict):
    ...
    '''
    print(f"Scale file {url_dict[year]}")
...
def get_SS_runbins_and_SS_dict(year,url_dict):
    newdf=getSSdictRun2(year,url_dict)
    runbins=[]
